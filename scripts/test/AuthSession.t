...
for my $ModuleFile (@BackendModuleFiles) {
    ...
    # Протухание сессий в Redis'е основано на внутреннем TTL Redis'а,
    # который не связан с DateTime. Поэтому тут или реальную паузу надо ставить
    # (что сильно затормозит процесс тестирования), либо отключать тест -- что и сделано
    if ( $Module ne 'Redis' ) {
